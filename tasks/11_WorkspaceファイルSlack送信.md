...
- Slack添付ファイル受信時に追記したファイルメタデータは自然文の送信対象パスとして扱わず、ユーザーからエージェントへ送ったファイルを誤ってSlack送信命令にしない。
...
- 添付付きメッセージ本文に「送って」などが含まれていても、添付ファイル名や保存パスをworkspaceファイル送信要求として誤検知しない。
