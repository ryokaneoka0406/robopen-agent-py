...
- 既存タスクの削除は、Slackの自然文または `schedule delete <task_id>` からWrapperが対象を特定し、確認メッセージを返す。ユーザーが同じSlack会話で `schedule delete confirm <task_id>` を送った場合のみ `tasks.status=cancelled` に更新し、Schedulerのhandleを解除する。監査性と復旧容易性のため物理DELETEは行わない。
